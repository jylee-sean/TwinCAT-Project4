...
HRESULT CModule1::SetObjStateSO()
{
	...
	for (auto motor = motors.begin(); motor != motors.end(); motor++) {
		...
		motor->SetTargetPosition(m_DataArea3.target_position);
	...
	for (auto motor = motors.begin(); motor != motors.end(); motor++) {
		 ...
		 {			
			 case state::enable:
				 ...
				 motor->m_re_execute = motor->m_execute;
				 break;
			 case state::set:
