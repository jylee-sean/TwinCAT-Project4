...
<<<<<<< HEAD
	/* cmd : on */
	trainsitionMap[setShutdown] = &CModule1::set_shutdown;
	trainsitionMap[setSwitchOn] = &CModule1::set_switch_on;
	trainsitionMap[setEnabled] = &CModule1::set_enable;
	trainsitionMap[Enabled] = nullptr; /*enabled*/
	/* cmd : set */
	trainsitionMap[setPosition] = &CModule1::set_position;
	trainsitionMap[setPoint] = &CModule1::set_point;
	trainsitionMap[tgtReached] = &CModule1::clear_set_point; 
	trainsitionMap[moving] = nullptr;  
	trainsitionMap[clearPoint] = &CModule1::clear_set_point;

	/* cmd : stop */
	trainsitionMap[quickstop] = &CModule1::set_quick_stop;


	trainsitionMap[setDisabled] = &CModule1::set_disable_voltage;
	trainsitionMap[faultReset] = &CModule1::set_fault_reset;
	trainsitionMap[Disabled] = nullptr; /*disabled*/
...
=======
		...
		//auto cmd = this->GetCommand(*motor);
		//auto state = this->GetState(*motor);
		
		USHORT state = 0; /*will be removed*/

						  
		if (state != expected_state[cmd] ) {
			// state -> int
			exe = this->arr[cmd][state]; 
		}
		else if ((state & status_word::operation_enabled) == status_word::operation_enabled){
			
			/*if new position is set*/

		
			exe = this->move[motor->m_seq];

			if (exe == nullptr) {
				motor->m_seq = 0;
				return hr;
			}
			else if ((this->*exe)(*motor)) {
				motor->m_seq++;
			}
			return hr;
		}
